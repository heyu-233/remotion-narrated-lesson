...
description: |
  把"用户预先写好的口播稿 + 用户预先录好的旁白录音"做成一支带动画的类 PPT 教学视频，工程基于 Remotion，时间线由 whisper 转写出的 segment 时间戳驱动。适用于编程、系统、数据库、AI、嵌入式等计算机类教学视频。
  本 skill 不再假定只有一种开发模式，而是先判断入口，再走对应流程：
  - 章节课件流（录音先行，按 Chapter 开发）
  - 代码讲解流（单工作区 / 单编辑器视图 / 音频驱动字幕与代码高亮）
  - 混合叙事流（代码讲解 + 章节动画并存）
  触发词：
...
把口播稿和旁白录音变成"看起来不像 PPT 但本质是 PPT"的教学视频。**录音是真理来源，动画追随录音**。但不要再默认所有项目都走同一条 Chapter 流水线。

适用：编程、系统、数据库、AI、嵌入式等计算机类中文教程视频，5–10 分钟一集，1920×1080@30fps，B 站/视频号/YouTube 录屏发布。

## 入口先决策：先选开发路径

开始实现前，先判断当前项目属于哪一种模式：

| 模式 | 适用场景 | 典型输入 | 主要输出 |
|---|---|---|---|
| **章节课件流** | 用户要做章节化教学视频，每章画面策略可能不同 | `script.md` + `voiceover.*` + `outline.md` | `ChapterN.tsx` + 章节驱动的 `Composition.tsx` |
| **代码讲解流** | 用户已经有代码工作区，要做“边讲边高亮边滚动”的代码 walkthrough | 代码工程 + `voiceover.*` + 可选 `script.md` | 单编辑器主视图、字幕真值层、cue sheet、稳定视窗 |
| **混合叙事流** | 前半段代码讲解，后半段切图示/章节动画 | 代码工程 + 录音 + 提纲 | 代码窗 + 章节动画的混合结构 |

### 模式选择规则

- 用户明确说“纯代码演示”“先把字幕和代码对齐”“就是代码讲解部分”时，默认走 **代码讲解流**。
- 用户明确说“按章节做动画”“每章想做不同视觉模板”时，默认走 **章节课件流**。
- 两者同时存在时，走 **混合叙事流**，但实现顺序应先把代码讲解段做稳，再扩到章节动画。

### 禁止事项

- 禁止把 **章节课件流** 当成所有项目的默认入口。
- 禁止在明显属于 **代码讲解流** 的项目里，强行要求先拆 `Chapter1.tsx ~ Chapter8.tsx` 再开工。
- 禁止在 **代码讲解流** 里把 `script.md` 当成屏幕字幕真值；它只用于校对明显错词。
...
以下规则来自实际教学视频制作，适用于三种开发路径，优先级高于“先做一张章节页面再加淡入”的默认做法。

### 1. 时间真值与可见字幕分离

- Whisper 的 `start/end` 是音频对齐证据，但 ASR 文本不是屏幕字幕真值。
- 保持时间戳不动，建立 `subtitleTruth.ts` 或等价的人审文本层；逐句修正技术术语、命令、路径、缩写和错别字。
- 字幕按实际说话短句切分，通常 0.8–3.5 秒；不要把一段口播压缩成一条“概括字幕”。
- 字幕未命中时必须为空。不得重复上一句，也不得用章节摘要填补停顿。

### 2. 录屏是局部时间线

- 一条录屏只演示一个教学动作：干净初始状态、输入、关键输出、短暂停留。禁止“一条命令跑完全部验证”。
- 每条 `OffthreadVideo` 必须被放在 `Sequence from={shotStartFrame}` 中，确保它从录屏第 0 帧开始，而不是从整片视频的全局帧开始。
- 录制前清掉旧滚屏、密码、无关窗口、构建等待、主机警告和桌面内容。先录最小片段，再按口播拼接。
- 结果定格必须冻结同一视频源的精确帧。禁止用黑幕伪造空状态，禁止从运动视频突然换到无关海报图。
...
2. 保留命令和输出自然出现的过程。
3. 找到第一个“结果完整”的源视频帧。
4. 冻结该帧，然后在 1.8–3.5 秒内推近具体结果区域。
5. 用 `transformOrigin` 指向真实结果，不要默认从画布中心缩放。
6. 推镜后检查命令前缀、输出和字幕安全区是否被裁切。

使用 `snippets/screen-recording/ScreenRecordingComposition.tsx` 中的 `TimedCameraFreezeRecording`，不要重新手写这套逻辑。

### 4. 概念动画先原型，后帧驱动

- 当“这一段到底怎么动”还不明确时，先做独立 HTML/SVG 原型，验证镜头、图标、数据流与节奏。
- HTML 原型只负责视觉机制；最终 Remotion 必须重写为 `frame/seconds` 的纯函数。禁止把 CSS `transition`、墙钟 `@keyframes`、timer、hover/click 依赖直接带进最终成片。
- 每个概念镜头拆为 1.5–5 秒微镜头。每个微镜头必须发生可见事件：出现、阻断、抖动、重置、装配、数据包传输、状态变化或镜头聚焦。
- 用简洁 SVG 图标表达真实对象，如终端、文件、虚拟板、芯片、设备节点、网络端点。不要只堆文字卡片；流动箭头必须解释真实数据流或控制流。

### 5. 出片前强制检查

- 在 Studio 中连续播放，不只看 still 图；检查闪屏、黑场、错误裁切和录屏是否从局部第 0 帧开始。
- 抽查所有字幕边界，重点检查专有名词和口播停顿。
- 概念段每 1.5–5 秒至少有一次有意义的视觉变化；静态卡片长停需要明确理由。
- 完成后运行类型检查、时间线校验、媒体元数据检查，并生成 contact sheet。
...
- 用户已经建立好工作区
- 用户已经录好音频
- 视频主画面是代码编辑器 / IDE / 单工作区
- 目标不是“每章一套舞台”，而是“同一个代码窗口内稳定讲解”

### 代码讲解流的核心原则

1. **字幕真值和代码真值分离**
   - 字幕来自 `transcript.json` + 单独的文本修正层，例如 `subtitleTruth.ts`
   - 代码讲解节奏来自 cue sheet，例如 `narrationTruth.ts`
   - `script.md` 只用于纠正明显错词，不是屏幕字幕真值

2. **高亮系统和镜头系统分离**
   - 高亮可以跟着每句话切
   - 镜头不能跟着每句话乱跳
   - 默认先固定一个稳定的大视窗，再在视窗内移动高亮

3. **停顿区默认值必须干净**
   - 字幕未命中时必须空白
   - cue 未命中时必须保持上一镜头
   - 禁止回退到“最后一句字幕”或“最后一个 cue”

4. **想显示更多代码时，缩代码内容，不缩整个代码框**
   - 优先调整 `fontSize` / `lineHeight`
   - 不要优先缩放整个编辑器容器
...
- 读取 `voiceover.*`
- 生成或读取 `transcript.json`
- 确认是否已有现成代码工作区
- 确认是否已有现成 Remotion 代码演示工程
- 如果要新起一个代码讲解项目，优先复制：
  `templates-code-walkthrough/`

#### Step B2 — 建立字幕真值层

- 保持 `transcript.json` 的 `id/start/end` 不动
- 新建或维护 `subtitleTruth.ts`
- 只修明显错词、函数名、变量名、乱入句子
- 不把 `script.md` 改写成字幕

#### Step B3 — 建立 cue sheet

- 用 `narrationTruth.ts` 一类的数据层表达：
  - 当前讲哪个文件
  - 当前讲哪几行
  - 当前是否需要展示完整函数
  - 哪些片段属于同一个稳定视窗块

#### Step B4 — 先实现稳定视窗，再实现高亮

- 先决定“大视窗块”怎么切
- 同一块里默认不滚动，只换高亮
- 只有跨函数、跨代码块时才重新滚动或换构图

#### Step B5 — 清理默认值污染

- `findSegmentForFrame()` 未命中时返回 `null`
- `findSubtitleSegment()` 未命中时屏幕字幕为空
- `findNarrationCue()` 未命中时保持上一段镜头
...
- 禁止单行高亮就触发整屏大幅缩放
- 禁止把“高亮范围”直接等同于“镜头范围”
- 模板目录以 `templates-code-walkthrough/` 为准，不要误用章节课件流的 `templates/`

## 混合叙事流

如果项目既包含代码讲解，又包含章节化动画，则：

1. 代码讲解部分按 **代码讲解流** 做稳
2. 图示/章节动画部分按 **章节课件流** 增量补入
3. 不要一开始就把整个项目强拆成纯 Chapter 模型
...
- `字幕未命中 = 空白字幕`，禁止默认回退到最后一句字幕。
- `镜头 cue 未命中 = 保持上一镜头`，禁止默认回退到最后一个 cue。
- 在代码讲解流里，`script.md` 只用于核对明显错词，不是屏幕字幕真值。
